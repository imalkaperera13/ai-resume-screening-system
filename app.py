...
def calculate_final_score(text_score, skill_score):
    return round((text_score * 0.7) + (skill_score * 0.3), 2)
...
if st.button("Analyze Resumes"):
    if uploaded_resumes and job_description:
        ...
        for uploaded_resume in uploaded_resumes:
            ...
            text_score = calculate_text_similarity(cleaned_resume, cleaned_jd)
            ...
            skill_score = calculate_skill_match_score(matched_skills, jd_skills)
        ...
        avg_score = round(results_df["Final Score (%)"].mean(), 2)

        st.subheader("Dashboard Summary")
        col1, col2, col3 = st.columns(3)
        col1.metric("Total Resumes", len(results_df))
        col2.metric("Top Score", f"{top_candidate['Final Score (%)']}%")
        ...
        ax.bar(results_df["Resume Name"], results_df["Final Score (%)"])
        ax.set_xlabel("Resume Name")
        ax.set_ylabel("Final Score (%)")
        ax.set_title("Resume Ranking by Final Score")
        ...
        for _, row in top_3.iterrows():
            st.info(
                f"Rank {row['Rank']} - {row['Resume Name']} - Final Score: {row['Final Score (%)']}% - {row['Recommendation']}"
            )
        ...
        st.success(
            f"Best Match: {top_candidate['Resume Name']} with {top_candidate['Final Score (%)']}% "
